...
function PromptCard({ post, handleTagClick, handleEdit, handleDelete }) {
  console.log(post, "from prompt card");
    ...
    <div className="prompt_card bg-slate-50">
      <div className="flex justify-between items-start gap-5">
        <div className="flex-1 flex justify-start items-center gap-3 cursor-pointer">
          ...
          <div className="flex flex-col">
            <h3 className="font-satoshi  capitalize font-semibold text-gray-900">
              {post?.creator?.name}
            </h3>
      ...
      <div className="flex flex-col">
        <h3 className="font-bold  capitalize orange_gradient text-center mt-1">
          {" "}
          {post.bookName ? ` ${post.bookName}` : ""}
        </h3>
        <p className="text-xs  capitalize text-center font-inter">
          {post.authour ? `-by ${post.authour}` : ""}
        </p>
      </div>
